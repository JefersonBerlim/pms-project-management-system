...
public class TbTipopessoaJpaController implements Serializable {
    ...
    public TbTipopessoaJpaController(UserTransaction utx, EntityManagerFactory emf) {
        this.utx = utx;
        this.emf = emf;
    }
    ...
    private List<TbTipopessoa> findTbTipopessoaEntities(boolean all, int maxResults, int firstResult) {
        ...
        try {
            ...
            cq.select(cq.from(TbTipopessoa.class));
    ...
    public int getTbTipopessoaCount() {
        ...
        try {
            CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
            Root<TbTipopessoa> rt = cq.from(TbTipopessoa.class);
