...
public class TbDiaSemanaJpaController implements Serializable {
    ...
    public TbDiaSemanaJpaController(UserTransaction utx, EntityManagerFactory emf) {
        this.utx = utx;
        this.emf = emf;
    }
    ...
    private List<TbDiaSemana> findTbDiaSemanaEntities(boolean all, int maxResults, int firstResult) {
        ...
        try {
            ...
            cq.select(cq.from(TbDiaSemana.class));
    ...
    public int getTbDiaSemanaCount() {
        ...
        try {
            CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
            Root<TbDiaSemana> rt = cq.from(TbDiaSemana.class);
